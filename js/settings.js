...
(function (global) {
  ...
  function bindEvents() {
    ...
    document.getElementById('saveServerUrlBtn').addEventListener('click', function () {
      const value = document.getElementById('serverUrlInput').value.trim();
      Storage.setServerUrl(value);
      UI.toast(value ? 'تم حفظ رابط الخادم ✅' : 'تم مسح رابط الخادم.', 'success');
    });
  }
  ...
  function init() {
    ...
    refreshConnectionBadge();
    document.getElementById('serverUrlInput').value = Storage.getServerUrl();
    bindEvents();
  }

...
(function (global) {
  ...
  // يحدد نوع نقطة النهاية بناءً على اسم الـ Model المُدخل من الإعدادات.
  // قابل للتوسعة مستقبلًا لدعم مزودين آخرين (OpenAI، Claude عبر الخادم...).
  ...
  async function generateQuestion(params) {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        const rawText = await callModel({ apiKey, model, prompt });
  ...
  async function testConnection() {
    const apiKey = Storage.getApiKey();
    const model = Storage.getModel();
    if (!apiKey) return { success: false, message: 'لم تتم إضافة API Key بعد.' };
    ...
    try {
      const text = await callModel({ apiKey, model, prompt: testPrompt });

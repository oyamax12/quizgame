...
(function () {
  ...
  function navigateTo(id) {
    ...
    if (id === 'screen-mp-lobby') renderMpLobby();
  ...
  document.addEventListener('click', function (e) {
    ...
    if (navBtn) {
      ...
      if (navBtn.dataset.mpMode) pendingMpMode = navBtn.dataset.mpMode;
  ...
  function renderMpLobby() {
    ...
    document.getElementById('mpNoServer').hidden = !!serverUrl;
    document.getElementById('mpLobbyActions').hidden = !serverUrl;
    const isDuel = pendingMpMode === 'duel';
    document.getElementById('mpLobbyTitle').textContent = isDuel ? '🌐 Online 1v1' : '👥 لعب مع الأصدقاء';
    document.querySelector('#mpLobbyActions h3').textContent = isDuel ? 'إنشاء مباراة جديدة' : 'إنشاء غرفة جديدة';
    document.getElementById('mpCreateBtn').textContent = isDuel ? 'إنشاء مباراة 🚀' : 'إنشاء غرفة 🚀';
  }

  async function ensureMpConnection() {
    if (Multiplayer.isConnected()) return true;
    const serverUrl = Storage.getServerUrl();
    if (!serverUrl) { UI.toast('أضف رابط الخادم من الإعدادات أولًا.', 'error'); return false; }
    try {
      mpConnecting = true;
      await Multiplayer.connect(serverUrl);
      return true;
    } catch (err) {
      UI.toast(err.message || 'تعذر الاتصال بالخادم.', 'error');
      return false;
    } finally {
      mpConnecting = false;
    }
  ...
  document.getElementById('mpCreateBtn').addEventListener('click', async function () {
    if (mpConnecting) return;
    const ok = await ensureMpConnection();
    if (!ok) return;
    const gs = Storage.getGameSettings();
    Multiplayer.createRoom({
      name: Storage.getPlayerName(),
      avatar: Storage.getPlayerAvatar(),
      mode: pendingMpMode,
      config: { questionCount: gs.questionCount, timerDuration: gs.timerDuration }
    });
  });
  ...
  document.getElementById('mpJoinBtn').addEventListener('click', async function () {
    ...
    if (!code) { UI.toast('أدخل رمز الغرفة أولًا.', 'error'); return; }
    const ok = await ensureMpConnection();
    if (!ok) return;
    Multiplayer.joinRoom({ code: code, name: Storage.getPlayerName(), avatar: Storage.getPlayerAvatar() });
  });

  Multiplayer.on('error', function (data) { UI.toast(data.message || 'حدث خطأ في الخادم.', 'error'); });
  Multiplayer.on('disconnected', function () {
    const activeScreen = document.querySelector('.screen.is-active');
    if (activeScreen && ['screen-mp-room', 'screen-mp-game'].includes(activeScreen.id)) {
      UI.toast('انقطع الاتصال بالخادم.', 'error');
    }
  });

  function renderMpRoom(players) {
    document.getElementById('mpPlayerCount').textContent = players.length;
    const me = players.find(function (p) { return p.id === Multiplayer.getPlayerId(); });
    const list = document.getElementById('mpPlayersList');
    list.innerHTML = players.map(function (p) {
      const badges = [
        p.isHost ? '<span class="badge is-host">👑 المضيف</span>' : '',
        p.ready ? '<span class="badge is-ready">جاهز ✅</span>' : '<span class="badge">بالانتظار</span>'
      ].join('');
      return `<div class="mp-player-row${p.connected ? '' : ' is-disconnected'}"><span class="avatar">${p.avatar}</span><span class="name">${p.name}${p.id === Multiplayer.getPlayerId() ? ' (أنت)' : ''}</span>${badges}</div>`;
    }).join('');

    const readyBtn = document.getElementById('mpReadyBtn');
    readyBtn.textContent = me && me.ready ? 'إلغاء الاستعداد ❌' : 'أنا جاهز ✅';
    readyBtn.dataset.ready = me && me.ready ? '1' : '0';
  }

  ['roomCreated', 'joinedRoom'].forEach(function (evt) {
    Multiplayer.on(evt, function (data) {
      document.getElementById('mpRoomCode').textContent = data.code;
      navigateTo('screen-mp-room');
      renderMpRoom(data.players);
    });
  });
  Multiplayer.on('playersUpdate', function (data) { renderMpRoom(data.players); });

  document.getElementById('mpReadyBtn').addEventListener('click', function () {
    const nowReady = this.dataset.ready !== '1';
    Multiplayer.setReady(nowReady);
  });

  document.getElementById('mpCopyCodeBtn').addEventListener('click', async function () {
    const code = document.getElementById('mpRoomCode').textContent;
    try { await navigator.clipboard.writeText(code); UI.toast('تم نسخ الرمز 📋', 'success'); }
    catch (e) { UI.toast(code, 'success'); }
  });

  document.getElementById('mpLeaveBtn').addEventListener('click', function () {
    Multiplayer.disconnect();
    navigateTo('screen-home');
  });

  Multiplayer.on('gameStarting', function () {
    mpAnswered = false;
    navigateTo('screen-mp-game');
  });

  Multiplayer.on('question', function (q) {
    mpAnswered = false;
    document.getElementById('mpProgress').textContent = (q.index + 1) + ' / ' + q.total;
    document.getElementById('mpQuestionText').textContent = q.question;
    document.getElementById('mpExplanationBox').hidden = true;
    document.getElementById('mpPlayersStrip').innerHTML = '';

    const grid = document.getElementById('mpOptionsGrid');
    grid.innerHTML = '';
    q.options.forEach(function (opt, idx) {
      const btn = UI.el(`<button class="option-btn" data-index="${idx}">${opt}</button>`);
      btn.addEventListener('click', function () {
        ...
        mpAnswered = true;
        Multiplayer.answer(idx);
        document.querySelectorAll('#mpOptionsGrid .option-btn').forEach(function (b) {
          b.classList.add('is-disabled');
          if (Number(b.dataset.index) === idx) b.classList.add('is-correct');
        });
        UI.Sound.click();
      });
      grid.appendChild(btn);
    ...
    const ring = document.getElementById('mpTimerRing');
    document.getElementById('mpTimerValue').textContent = remaining;
    mpTimerInterval = setInterval(function () {
      remaining--;
      document.getElementById('mpTimerValue').textContent = Math.max(0, remaining);
      ring.classList.toggle('is-low', remaining <= 5);
  ...
  Multiplayer.on('roundResult', function (data) {
    clearInterval(mpTimerInterval);
    document.querySelectorAll('#mpOptionsGrid .option-btn').forEach(function (b) {
      b.classList.add('is-disabled');
      b.classList.remove('is-correct');
      if (Number(b.dataset.index) === data.correctAnswer) b.classList.add('is-correct');
    });

    const me = data.players.find(function (p) { return p.id === Multiplayer.getPlayerId(); });
    if (me) {
      document.getElementById('mpScore').textContent = me.totalScore;
      me.correct ? UI.Sound.correct() : UI.Sound.wrong();
    }

    document.getElementById('mpExplanationText').textContent = data.explanation || 'لا يوجد شرح لهذا السؤال.';
    document.getElementById('mpRoundScores').innerHTML = data.players.map(function (p) {
      return `<div class="row ${p.correct ? 'is-correct' : 'is-wrong'}"><span>${p.avatar} ${p.name}</span><strong>${p.correct ? '+' + p.roundPoints : '0'} (${p.totalScore})</strong></div>`;
    }).join('');
    document.getElementById('mpExplanationBox').hidden = false;
  });

  Multiplayer.on('gameOver', function (data) {
    const medals = ['🥇', '🥈', '🥉'];
    document.getElementById('mpFinalRanking').innerHTML = data.players.map(function (p, i) {
      return `<div class="row"><span class="rank">${medals[i] || (i + 1)}</span><span>${p.avatar} ${p.name}${p.id === Multiplayer.getPlayerId() ? ' (أنت)' : ''}</span><span class="score">${p.score}</span></div>`;
    }).join('');
    navigateTo('screen-mp-results');
  });

  // ---------- المتصدرون العالميون ----------
  async function renderLeaderboard() {
    const serverUrl = Storage.getServerUrl();
    document.getElementById('leaderboardNoServer').hidden = !!serverUrl;
    const listEl = document.getElementById('leaderboardList');
    listEl.innerHTML = '';
    if (!serverUrl) return;
    try {
      const res = await fetch(toHttpUrl(serverUrl) + '/leaderboard');
      const list = await res.json();
      const medals = ['🥇', '🥈', '🥉'];
      listEl.innerHTML = list.length
        ? list.map(function (p, i) {
          return `<div class="row"><span class="rank">${medals[i] || (i + 1)}</span><span>${p.avatar} ${p.name}</span><span class="score">${p.score}</span></div>`;
        }).join('')
        : '<p class="hint-text">لا توجد نتائج مسجّلة بعد. العب مباراة جماعية أولًا!</p>';
    } catch (e) {
      listEl.innerHTML = '<p class="hint-text">تعذر جلب المتصدرين من الخادم. تحقق من الرابط ومن أن الخادم يعمل.</p>';
    }
  }
